...
Second, we've marked places in this file where you are you fill
...
think creatively and useful everything you've learned in this class so
...
set_forall: (eq a) => (p: a -> bool) -> (s: set a) -> bool


-- Return true if p is true for some v in s, otherwise else false
set_exists: (eq a) => (p: a -> bool) -> (s: set a) -> bool
...
set_witness: (eq a) => (p: a -> bool) -> (s: set a) -> option a
...
-- If v is in s, return true, otherwise return false
...
list_difference nil l2 = nil
list_difference (h::t) nil = (h::t)
...
-- Return true if p is true for every v in s, otherwise return false
-- set_forall: (eq a) => (p: a -> bool) -> (s: set a) -> bool

set_forall p (mkSet l) = eql l (filter p l)

-------------------------------------

-- Return true if p is true for some v in s, otherwise return false
-- set_exists: (eq a) => (p: a -> bool) -> (s: set a) -> bool

set_exists p (mkSet l) = not (isEmpty (mkSet (filter p l)))
...
choose_one: (eq a) => list a -> a
choose_one (h::t) = h

list_witness: (eq a) => (a -> bool) -> list a -> option a
list_witness p l = ite (set_exists p (mkSet (filter p l)))
                    (some (choose_one (filter p l)))
                    (none)
...
-- pair elt takes an elt h and a list b and returns list of all
-- pairs, (h, e), where e is an element of list b
...
set_toString (mkSet l) = "{" ++ (toStringList l) ++ "}"
